...
const Map = () => {
	...
	useEffect(() => {
		const values = async () => {
			console.log("진입");
			const response = await axios.get("/api/map/location");
	...
	// const submitPlace = () => {
	//   axios
	//     .post("http://localhost:4000/api/insert", {
	//       number: 0,
	//       longitude: 0,
	//       latitude: 0,
	//     })
	...
	const mapscript = async () => {
		...
		place.forEach((el) => {
			// console.log(el.geolocation, "elecemet check");
			// 마커를 생성합니다
	...
	return (
		<>
			...
			<button onClick={openClick}>hello openfaas</button>
		</>
	);
};
